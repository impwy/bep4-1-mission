...
public class PayoutAddPayoutCandidateItemUseCase {
    ...
    private void makePayoutCandidateItems(OrderDto order, OrderItemDto orderItem) {
        ...
        PayoutMember buyer = payoutSupport.findPayoutMember(orderItem.getBuyerId()).get();
        PayoutMember seller = payoutSupport.findPayoutMember(orderItem.getSellerId()).get();
    ...
    private void makePayoutCandidateItem(PayoutEventType eventType, String relTypeCode, int relId,
                                         LocalDateTime paymentDate, PayoutMember payer, PayoutMember payee,
                                         long amount) {
        PayoutCandidateItem payoutCandidateItem = new PayoutCandidateItem(eventType, relTypeCode, relId,
                                                                          paymentDate, payer, payee, amount);
